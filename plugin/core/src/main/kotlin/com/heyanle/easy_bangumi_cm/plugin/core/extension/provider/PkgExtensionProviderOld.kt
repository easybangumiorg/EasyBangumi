...
class PkgExtensionProviderOld(
    ...
    private suspend fun innerLoad(
        ...
        for (mutableEntry in conflict) {
            ...
            list.forEach {
                if (it != max) {
                    File(it.workPath).deleteRecursively()
                }
            }
        ...
        for (manifest in needRename) {
            ...
            // rename folder
            val folder = File(manifest.workPath)
            ...
            val newManifest = manifest.copy(
                ...
                workPath = newFolder.absolutePath,
        ...
        // index
        for (re in res) {
            FolderIndex.make(re.value.workPath, re.value.lastModified)
        }
