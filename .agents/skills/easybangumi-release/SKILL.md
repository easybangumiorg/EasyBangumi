...
5. **Built-in sources:** require `inner_source/` at the repository root and no source files under `app/src/main/assets/inner_source/`. Ensure `app/build.gradle.kts` generates `assets/inner_source/` from the root directory. Move duplicate source files to the root only after checking content equality; do not overwrite conflicting files without user direction.
