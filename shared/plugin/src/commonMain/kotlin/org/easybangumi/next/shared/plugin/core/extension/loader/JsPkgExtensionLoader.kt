...
class JsPkgExtensionLoader(
    ...
    private val jsCryExtensionLoader: JsFileCryExtensionLoader,
): ExtensionLoader {
    ...
    override suspend fun load(extensionManifest: ExtensionManifest): ExtensionInfo {
        ...
        val sourceManifestList = jsFileFolder.listFiles().filter { it?.isFile() == true }.filterNotNull()
            .mapNotNull {
                if (it.getName().endsWith(".jsc")) {
                    return@mapNotNull jsCryExtensionLoader.loadFile(extensionManifest, it)
                } else {
                    return@mapNotNull jsFileExtensionLoader.loadFile(extensionManifest, it)
                }
            }
