...
// @versionCode 10000
// @libVersion 15
// @cover https://www.bmmdm.com/hdst/hm_pic/favicon.ico
...
var DEFAULT_BASE_URL = "https://www.baimaodm.com/";
var SEARCH_URL = "https://www.baimaodm.com/s_all?ex=1&kw=@keyword";
var SEARCH_LIST_XPATH = "//div[4]/div[2]/div[1]/ul/li";
var SEARCH_NAME_XPATH = "//h2/a";
var SEARCH_RESULT_XPATH = "//h2/a";
var CHAPTER_ROADS_XPATH = "//div[2]/div[2]/div[8]/div/div/ul";
...
// The tab labels above #main0 identify the corresponding .movurl lists.
function getPlayLineName(doc, index) {
    var tabs = doc.select("#menu0 li");
    if (tabs != null && index < tabs.size()) {
        var name = trimText(tabs.get(index).text());
        if (name.length > 0) {
            return name;
        }
    }
...
function getDetailDescription(doc) {
    var selectors = [".tabs .info", "meta[name=description]"];
    for (var i = 0; i < selectors.length; i++) {
