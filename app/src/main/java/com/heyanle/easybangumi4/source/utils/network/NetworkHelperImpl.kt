...
class NetworkHelperImpl(
    ...
    // 这里获取默认 UA 为耗时操作，缓存一波
    private var defaultAndroidUAOKKV by okkv<String>("defaultAndroidUA", "")
    override val defaultAndroidUA: String
        get() {
            if (defaultAndroidUAOKKV.isEmpty()) {
                ...
                return defaultAndroidUAOKKV
            }
        }

    override val defaultLinuxUA: String
        get() =  "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36 Edg/108.0.1462.76"

    override val randomUA: String
        get() = NetworkHelperImpl.USER_AGENT_ARRAY.random()
}