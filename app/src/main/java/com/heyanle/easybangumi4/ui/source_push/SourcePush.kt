...
// 先放弃拓展性，直接写死
sealed class SourcePushType(
    val identify: String,
    val label: @Composable ()-> String,
    val desc: @Composable ()-> String,
) {

    data object FromFileUrl: SourcePushType(
        identify = SourcePushTask.SOURCE_PUSH_TASK_IDENTIFY_FROM_FILE_URL,
        label = @Composable { stringResource(R.string.js_file_url) },
        desc = @Composable { stringResource(R.string.js_file_url_desc) }
    )

    data object FromCode: SourcePushType(
    ...
    data object FromRepo: SourcePushType(
        identify = SourcePushTask.SOURCE_PUSH_TASK_IDENTIFY_FROM_FILE_REPO,
...
val sourcePushTypeList: List<SourcePushType> by lazy {
    listOf(
        SourcePushType.FromFileUrl,
        SourcePushType.FromCode,
    )
}
...
fun SourcePush(
    ...
    val vm = viewModel<SourcePushViewModel>()
    ...
    when(sta.dialog) {
        is SourcePushViewModel.Dialog.Loading -> {
            AlertDialog(
                onDismissRequest = {
                    //vm.cancelCurrent()
                },
        ...
        is SourcePushViewModel.Dialog.ErrorOrCompletely -> {
            AlertDialog(
                onDismissRequest = {
