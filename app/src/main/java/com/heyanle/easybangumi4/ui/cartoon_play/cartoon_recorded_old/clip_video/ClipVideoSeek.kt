...
fun ClipVideoSeek(
    ...
    val pos2PxParam = state.getPos2PxParam()
    val px2PosParam = state.getPx2PosParam()
    ...
    LaunchedEffect(key1 = Unit) {
        ...
        launch {
            snapshotFlow {
                ...
                val pngWidth = state.height
                val pngCount = ceil((state.width - 2 * state.horizontalPadding).toDouble() / pngWidth).toInt()

                var currentPx = state.horizontalPadding
                val arrayList = arrayListOf<Long>()
                repeat(pngCount){
                    val currentPosition = (currentPx.toLong() * px2PosParam.first + px2PosParam.second).toLong()
                    if (currentPosition >= state.start && currentPosition <= state.end) {
        ...
        .pointerInput("", true) {
            detectHorizontalDragGestures(
                onDragStart = {
                    val diff = 200
                    // current
                    val currentPx = state.currentPosition * pos2PxParam.first + pos2PxParam.second
...
//                            }
                            OkImage(
                                ...
                                modifier = Modifier
                                    .height(state.height.dip2px().dp)
                                    .width(state.height.dip2px().dp)
                            )
                        }
