...
class ExtensionStoreInfoRepository {
    ...
    suspend fun getInfoList(): DataResult<ExtensionStoreInfo> {
        return withContext(Dispatchers.IO) {
            ...
            if (!resp.isSuccessful || body == null) {
                return@withContext DataResult.error<ExtensionStoreInfo>(resp.message)
            }
            val info = body.jsonTo<ExtensionStoreInfo>()
            if(info == null || info.apiVersion > EXTENSION_STORE_VERSION){
                return@withContext DataResult.error<ExtensionStoreInfo>(stringRes(com.heyanle.easy_i18n.R.string.easybangumi_too_old))
            }
