...
object LocalItemFactory {
    ...
    @WorkerThread
    suspend fun newItemFolder(cartoonInfo: CartoonInfo, label: String, rootFolder: UniFile): UniFile? {
        var targetFolder = rootFolder.findFile(label)
        if (targetFolder != null && targetFolder.exists()){
            targetFolder.deleteRecursively()
        }
        targetFolder = rootFolder.createDirectory(label) ?: return null
        ...
        tvShow.appendElement("title").text(cartoonInfo.name)
        tvShow.appendElement("plot").text(cartoonInfo.description)
        tvShow.appendElement("art").appendElement("poster").text(if (hasPng)"cover.png" else cartoonInfo.coverUrl)
