...
class EasyPlayer: JzvdStd {
    ...
    override fun clickClarity() {
        ...
        val mQualityListener = OnClickListener { v1: View ->
            ...
            for (j in 0 until linear.childCount) { //设置点击之后的颜色
                if (j == getCurrentIndex()) {
                    (linear.getChildAt(j) as TextView).setTextColor(getAttrColor(jzvdContext, com.google.android.material.R.attr.colorSecondary))
                } else {
        ...
        for (j in episodeList.indices) {
            ...
            if (j == getCurrentIndex()) {
                clarityItem.setTextColor(getAttrColor(jzvdContext, com.google.android.material.R.attr.colorSecondary))
            }
        }
        clarityPopWindow =
        ...
        clarityPopWindow.animationStyle = cn.jzvd.R.style.pop_animation
